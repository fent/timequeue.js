...
TimeQueue.prototype._process = function(args) {
  ...
  if (every) {
    ...
    self._timers.push(setTimeout(function everyTimeout() {
      timedOut = true;
      self._timers.shift();
      // go to next job only if current timeout has happened to avoid throttling from queue handler
      setTimeout(function(){
        if (finished) {
          self._next();
        }
      }, (every - (+new Date() - everyTime)));
    }, every));
  ...
  function taskCallback(err) {
    ...
    finished = true;
// go to next job only if current timeout has happened to avoid throttling
    setTimeout(function(){
      if (timedOut) {
        self._next();
      }
